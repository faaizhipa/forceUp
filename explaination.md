...
- **Purpose**: Chrome MV3 extension that accelerates Salesforce Lightning case handling and knowledge-site tooling (highlighter, notes, bookmarks) for Clarivate/ProQuest support engineers.
- **Surfaces & entry points**: `background.js` (service worker, context menus, backups), `popup.html/js` + `sidepanel` for settings, `content_script_exlibris.js` (ProQuest controller), `content_script.js` (legacy helpers), `content_script_highlighter.js` (workspace tools), `manifest.json` defines load order and domain scopes.
- **Data flow (case page)**: Navigation signals → `PageIdentifier` + `NavigationObserver` → `CaseContextWatcher` resolves `{caseId, caseNumber}` → `CaseDataStore` holds the single active case → Extractors (`CaseDataExtractor`, `CasePageDataExtractor`) scrape Lightning (with Shadow DOM traversal) → business logic (URLBuilder, timezone helpers) → UI modules (PersistentBanner, DynamicMenu, FieldHighlighter, CommentMemory, CharacterCounter).
- **Critical modules**: `CaseContextWatcher` + `CaseDataStore` (single source of truth), `NavigationObserver` (SPA safety), `CasePageDataExtractor` (heavy DOM traversal), `DynamicMenu` (external links), `CustomerTimezoneLookup`/`CustomerDataManager` (timezone correctness), `DataMigration` (workspace backups).
- **External dependencies**: `DOMPurify` (sanitization), `html2canvas` + `fabric` (screenshots/annotation), Chrome APIs (`contextMenus`, `storage`, `alarms`, `tabs`, `sidePanel`, `identity`), Google Drive OAuth for backups.
- **Practices to keep**: Check-then-observe for DOM, context-first validation via `PageContextValidator`, idempotent injection with cleanup, async/await + try/catch, avoid autogenerated selectors, prefer `chrome.storage.sync` for prefs and `local` for per-case artifacts, back up workspace data on install/update, tag injected DOM with `data-exl-*` for cleanup.
- **Risks & gaps**: No automated tests or selector monitors; `caseCommentExtractor` usage on ProQuest still undecided; UX specs for banners/menus are light; long-running observers or missing cleanup can leak; dynamic menu mappings must stay current; timezone data must match `instTimezones.dsv`.
- **Documentation hygiene**: Keep this snapshot aligned with reality, log every notable change in `CHANGES.md` (Date, Category, Description, Files, Lessons Learned, Related Issues), and update `copilot-instructions.md` when guidance shifts.
