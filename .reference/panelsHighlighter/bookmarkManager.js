...
const BookmarkManager = (function() {
  ...
  // Domains where Google Drive backup button is enabled
  const BACKUP_ENABLED_DOMAINS = ['force.com', 'clarivate.com', 'exlibrisgroup.com', 'proquest.com', 'google.com'];
  const BACKUP_FALLBACK_URL = 'https://support.clarivate.com';

  /**
   * Check if current domain allows Google Drive backup
   * @returns {boolean}
   */
  function isBackupEnabledDomain() {
    const hostname = window.location.hostname.toLowerCase();
    return BACKUP_ENABLED_DOMAINS.some(domain => hostname.endsWith(domain));
  }
  ...
  async function init() {
    ...
    // Setup storage change listener for real-time cross-tab updates
    setupStorageListener();
    
    // Listen for custom events from local updates
  ...
  /**
   * Setup storage change listener for real-time cross-tab updates
   * This ensures the panel updates immediately when data changes in other tabs
   */
  function setupStorageListener() {
    ...
    storageListener = (changes, areaName) => {
      ...
      // Check if bookmark-related storage changed
      const collectionsChanged = changes[keys.collections] || changes[oldCollectionKey];
      const bookmarksChanged = changes[keys.bookmarks] || changes[oldBookmarkKey];

      if (collectionsChanged || bookmarksChanged) {
        console.log('[BookmarkManager] Storage changed detected, reloading data...');
    ...
    console.log('[BookmarkManager] Storage listener setup complete');
  }

  /**
   * Setup custom event listeners for local updates
   * This ensures immediate updates when changes happen in the current tab
   */
  function setupCustomEventListeners() {
    // Listen for bookmark-related custom events
    window.addEventListener('exl-bookmark-created', () => {
      if (isPanelOpen) {
        loadData().then(() => refreshPanel());
      }
    });

    window.addEventListener('exl-bookmark-updated', () => {
      if (isPanelOpen) {
        loadData().then(() => refreshPanel());
      }
    });

    window.addEventListener('exl-bookmark-deleted', () => {
      if (isPanelOpen) {
        loadData().then(() => refreshPanel());
      }
    });

    window.addEventListener('exl-collection-created', () => {
      if (isPanelOpen) {
        loadData().then(() => refreshPanel());
      }
    });

    window.addEventListener('exl-collection-updated', () => {
      if (isPanelOpen) {
        loadData().then(() => refreshPanel());
      }
    });

    window.addEventListener('exl-collection-deleted', () => {
      if (isPanelOpen) {
        loadData().then(() => refreshPanel());
      }
    });

    console.log('[BookmarkManager] Custom event listeners setup complete');
  }
  ...
  async function saveCollections() {
    return new Promise((resolve) => {
      const keys = getStorageKeys();
      chrome.storage.local.set({ [keys.collections]: collections }, () => {
        if (chrome.runtime.lastError) {
  ...
  async function saveBookmarks() {
    return new Promise((resolve) => {
      const keys = getStorageKeys();
      chrome.storage.local.set({ [keys.bookmarks]: bookmarks }, () => {
        if (chrome.runtime.lastError) {
  ...
  async function renameCollection(collectionId, newName) {
    ...
    if (collections[collectionId]) {
      ...
      window.dispatchEvent(new CustomEvent('exl-collection-updated', { detail: { collectionId } }));
  ...
  async function deleteCollection(collectionId) {
    if (collections[collectionId]) {
      ...
      // Dispatch custom event for local updates
      window.dispatchEvent(new CustomEvent('exl-collection-deleted', { detail: { collectionId } }));
  ...
  /**
   * Show bookmark dialog for current page
   * Allows user to select a collection and bookmark the current page
   */
  function showBookmarkDialog() {
    ...
    if (collectionList.length === 0) {
      showToast('Please create a collection first before adding bookmarks', 'warning');
      // Optionally open the panel to create a collection
    ...
    const defaultCollectionId = collectionList[0].id;
    ...
    content.innerHTML = `
      <h3 style="margin: 0 0 1rem 0;">🔖 Bookmark This Page</h3>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">Collection:</label>
        <select id="bookmark-collection-select" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
          ${collectionList.map(c => `<option value="${escapeHtml(c.id)}" ${c.id === defaultCollectionId ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('')}
        </select>
      </div>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">Title:</label>
        <input type="text" id="bookmark-title" value="${escapeHtml(currentTitle.substring(0, 100))}" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;" maxlength="100">
      </div>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">URL:</label>
        <input type="url" id="bookmark-url" value="${escapeHtml(currentUrl)}" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;" readonly>
      </div>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">Description (optional):</label>
        <textarea id="bookmark-desc" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px; resize: vertical; min-height: 80px;" maxlength="200" placeholder="Add a note about this page..."></textarea>
      </div>
      <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
        <button id="bookmark-cancel" style="padding: 0.5rem 1rem; border: 1px solid #ddd; background: white; border-radius: 4px; cursor: pointer;">Cancel</button>
        <button id="bookmark-save" style="padding: 0.5rem 1rem; border: none; background: #0070d2; color: white; border-radius: 4px; cursor: pointer; font-weight: 600;">Save Bookmark</button>
      </div>
    `;
    ...
    // Event handlers
    const collectionSelect = content.querySelector('#bookmark-collection-select');
    const titleInput = content.querySelector('#bookmark-title');
    const urlInput = content.querySelector('#bookmark-url');
    const descInput = content.querySelector('#bookmark-desc');
    const cancelBtn = content.querySelector('#bookmark-cancel');
    const saveBtn = content.querySelector('#bookmark-save');
  ...
  async function createBookmark(collectionId, title, url, description = '') {
    ...
    // Dispatch custom event for local updates
    window.dispatchEvent(new CustomEvent('exl-bookmark-created', { detail: { bookmarkId, collectionId } }));
  ...
  async function deleteBookmark(bookmarkId) {
    if (bookmarks[bookmarkId]) {
      ...
      // Dispatch custom event for local updates
      window.dispatchEvent(new CustomEvent('exl-bookmark-deleted', { detail: { bookmarkId } }));
  ...
  async function updateBookmark(bookmarkId, updates) {
    ...
    // Dispatch custom event for local updates
    window.dispatchEvent(new CustomEvent('exl-bookmark-updated', { detail: { bookmarkId } }));
  ...
  async function duplicateBookmark(bookmarkId) {
    ...
    // Dispatch custom event for local updates
    window.dispatchEvent(new CustomEvent('exl-bookmark-created', { detail: { bookmarkId: newBookmarkId, collectionId: bookmark.collectionId } }));
   ...
   * This ensures data is reloaded from storage before rendering
   */
  function refreshPanel() {
    if (!isPanelOpen) return;
    
    // Reload data first to ensure we have the latest from storage
    loadData().then(() => {
  ...
  function renderPanel() {
    ...
    backupBtn.className = 'exl-hl-toolbar-btn exl-hl-btn-backup';
    backupBtn.id = 'exl-bm-btn-backup';
    backupBtn.innerHTML = '☁️ Backup';
    if (isBackupEnabledDomain()) {
      backupBtn.title = 'Backup to Google Drive';
    } else {
      backupBtn.title = 'Navigate to support.clarivate.com to use Google Drive backup';
      backupBtn.dataset.disabled = 'true';
    }
    backupBtn.addEventListener('click', handleBackupClick);
    ...
    // New collection button - moved to toolbar
    const newCollBtn = document.createElement('button');
    newCollBtn.className = 'exl-hl-toolbar-btn';
    newCollBtn.textContent = '+ New Collection';
    newCollBtn.title = 'Create a new collection';
    newCollBtn.style.cssText = 'margin-bottom: 0px; text-wrap: auto; font-size: 12px; padding-top: 6px; padding-bottom: 6px; background-color: rgb(44, 118, 204);';
    newCollBtn.addEventListener('click', showNewCollectionDialog);

    toolbar.appendChild(backupBtn);
    toolbar.appendChild(exportBtn);
    toolbar.appendChild(importBtn);
    toolbar.appendChild(newCollBtn);
  ...
  function showEditBookmarkDialog(bookmarkId) {
    ...
    content.innerHTML = `
      ...
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">Title:</label>
        <input type="text" id="edit-bookmark-title" value="${escapeHtml(bookmark.title)}" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;" maxlength="100">
      </div>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">URL:</label>
        <input type="url" id="edit-bookmark-url" value="${escapeHtml(bookmark.url)}" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px;">
      </div>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">Description:</label>
        <textarea id="edit-bookmark-desc" style="width: 100%; padding: 0.5rem; border: 1px solid #ddd; border-radius: 4px; resize: vertical; min-height: 80px;" maxlength="200">${escapeHtml(bookmark.description || '')}</textarea>
      </div>
      <div style="display: flex; gap: 0.5rem; justify-content: flex-end;">
  ...
  function showAddBookmarkDialog(collectionId) {
    ...
    content.innerHTML = `
      <h3 style="margin: 0 0 1rem 0;">Add Bookmark to "${escapeHtml(collections[collectionId].name)}"</h3>
      <div style="margin-bottom: 1rem;">
        <label style="display: block; margin-bottom: 0.25rem; font-weight: 600;">Title:</label>
  ...
  function createContextMenu(x, y, items) {
    ...
    items.forEach(item => {
      ...
      menuItem.addEventListener('click', () => {
        item.action();
        // Remove the click-outside listener before removing menu
        if (menu._closeMenuHandler) {
          document.removeEventListener('click', menu._closeMenuHandler);
        }
        menu.remove();
      });
    ...
    // Store closeMenu reference on menu for cleanup
   ...
   * @param {string} type - The toast type: 'info', 'success', 'warning', 'error' (default: 'info')
   */
   ...
   * If on enabled domain, trigger Google Drive backup
   * If not, redirect to support.clarivate.com
   */
  function handleBackupClick() {
    if (!isBackupEnabledDomain()) {
      // Redirect to support.clarivate.com
      const proceed = confirm(
        'Google Drive backup requires navigating to a Clarivate domain.\n\n' +
        'Would you like to open support.clarivate.com in a new tab?\n' +
        'You can use the backup feature from there.'
      );
      if (proceed) {
        window.open(BACKUP_FALLBACK_URL, '_blank');
      }
      return;
    }

    // On enabled domain - perform backup
    const backupBtn = document.getElementById('exl-bm-btn-backup');
    const originalText = backupBtn ? backupBtn.textContent : '☁️ Backup';
    
    if (backupBtn) {
      backupBtn.textContent = '☁️ Backing up...';
      backupBtn.disabled = true;
    }

    // Send message to background to trigger backup
    chrome.runtime.sendMessage({ type: 'RUN_DRIVE_BACKUP_FROM_SF' }, (response) => {
      if (chrome.runtime.lastError) {
        console.error('[BookmarkManager] Backup failed:', chrome.runtime.lastError.message);
        showToast('Backup failed: ' + chrome.runtime.lastError.message, 'error');
        if (backupBtn) {
      ...
      if (!response || !response.ok) {
        console.error('[BookmarkManager] Backup failed:', response?.error || 'Unknown error');
        showToast('Backup failed: ' + (response?.error || 'Unknown error'), 'error');
      } else {
        console.log('[BookmarkManager] Backup successful');
        showToast('Backup to Google Drive completed!', 'success');
      }
      ...
      <div class="exl-hl-dialog-content">
        ...
        <div class="exl-hl-dialog-section exl-hl-collection-select" style="display: none;">
          ...
          <select class="exl-hl-select" id="export-collection-select">
            ${collectionList.map(c => `<option value="${escapeHtml(c.id)}">${escapeHtml(c.name)} (${c.bookmarkIds.length} bookmarks)</option>`).join('')}
          </select>
        </div>
...
`;
    ...
    Object.values(data.collections).forEach(collection => {
      ...
      collection.bookmarkIds.forEach(id => {
        ...
        if (bookmark) {
          // Support both 'created' (internal) and 'createdAt' (imported) for backward compatibility
          const createdTimestamp = bookmark.created || bookmark.createdAt || Date.now();
          const addDate = Math.floor(new Date(createdTimestamp).getTime() / 1000);
  ...
  function showImportDialog() {
    ...
    fileInput.addEventListener('change', async (e) => {
      ...
      try {
        ...
        previewContent.innerHTML = `<p class="exl-hl-error">❌ ${escapeHtml(error.message)}</p>`;
        previewSection.style.display = 'block';
        importBtn.disabled = true;
        parsedData = null;
      }
  ...
  function parseHtmlBookmarks(html) {
    ...
    if (folders.length === 0) {
      ...
      const defaultCollId = 'imported_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
      ...
      doc.querySelectorAll('A').forEach((a, index) => {
        ...
        result.bookmarks[bookmarkId] = {
          ...
          created: addDate ? parseInt(addDate) * 1000 : Date.now()
        };
  ...
  async function performImport(data, conflictStrategy) {
    ...
    // Map old collection IDs to new collection IDs for bookmark lookup
    const collectionIdMap = {};
    // Track collections that use 'replace' strategy - need to rebuild bookmarkIds after import
    const replacedCollectionIds = new Set();
    
    // Import collections
    for (const [oldId, collection] of Object.entries(data.collections || {})) {
      ...
      if (existingCollection) {
        if (conflictStrategy === 'skip') {
          ...
          // Replace existing collection metadata and clear bookmarkIds to rebuild from imported bookmarks
          newId = existingCollection.id;
          collections[newId] = {
            ...collection,
            id: newId,
            bookmarkIds: [] // Clear bookmarkIds - will be rebuilt from imported bookmarks
          };
          collection.id = newId; // Update collection.id for consistency with other strategies
          replacedCollectionIds.add(newId);
          importedCollections++;
        } else if (conflictStrategy === 'rename') {
          // Create new with suffix
          newId = 'coll_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
          collections[newId] = {
            ...collection,
            id: newId,
      ...
      // Map old ID to new ID for bookmark lookup
    ...
    for (const [id, bookmark] of Object.entries(data.bookmarks || {})) {
      ...
      // Use the ID mapping to get the correct target collection ID
      const targetCollectionId = collectionIdMap[bookmark.collectionId] || bookmark.collectionId;
      ...
      if (existingBookmark) {
        if (conflictStrategy === 'skip') {
          ...
          // Add bookmark ID to collection's bookmarkIds if collection was replaced
          if (replacedCollectionIds.has(targetCollectionId) && collections[targetCollectionId]) {
            if (!collections[targetCollectionId].bookmarkIds.includes(existingBookmark.id)) {
              collections[targetCollectionId].bookmarkIds.push(existingBookmark.id);
            }
          }
          importedBookmarks++;
        } else if (conflictStrategy === 'rename') {
