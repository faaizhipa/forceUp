...
const PersistentBanner = {
    ...
    customerMetadata: {
        ...
        instCode: null, // Alias for institutionCode
        timezone: null
    },
    ...
    validateMessageText(text) {
        ...
        if (text.length > 20000) {
            return { valid: false, error: 'Message cannot exceed 20000 characters' };
        }
    ...
    applyDataUpdate(data, source = 'unknown') {
        ...
        if (data.institutionCode !== undefined || data.instCode !== undefined) {
            const newInstCode = data.institutionCode || data.instCode || null;
            if (newInstCode !== this.customerMetadata.institutionCode) {
                this.customerMetadata.institutionCode = newInstCode;
                this.customerMetadata.instCode = newInstCode; // Also set instCode alias for compatibility
                hasChanges = true;
            }
        }
    ...
    enrichCustomerMetadataFromManagers(accountName = null) {
        ...
        if (customerData) {
            if (!this.customerMetadata.customerId && (customerData.custID || customerData.customerId)) {
                this.customerMetadata.customerId = customerData.custID || customerData.customerId;
            }
            if (!this.customerMetadata.institutionId && (customerData.instID || customerData.institutionId)) {
                this.customerMetadata.institutionId = customerData.instID || customerData.institutionId;
            }
    ...
    setupCaseDataListener() {
        ...
        window.addEventListener('CASE_DATA_READY', async (event) => {
            ...
            if (caseData.server || caseData.customerId || caseData.institutionId || caseData.timezone || caseData.institutionCode) {
                this.customerMetadata = {
                    ...
                    instCode: caseData.institutionCode || caseData.instCode || null, // Also set instCode alias for compatibility
        ...
        document.addEventListener('casePageDataExtracted', async (event) => {
            ...
            const instCodeValue = data.institutionCode || data.instCode || null;
            this.customerMetadata = {
                ...
                institutionCode: instCodeValue,  // Institution code (61USC_INST, etc.)
                instCode: instCodeValue,  // Alias for institutionCode for compatibility
    ...
    clearCaseData(preserveContext = false) {
        ...
        this.customerMetadata = {
            ...
            productServiceName: null,
            institutionCode: null,
            instCode: null, // Also clear instCode alias
            timezone: null
        };
    ...
    addToNavigationHistory(pageInfo) {
        ...
        const historyEntry = {
            ...
            institutionCode: this.customerMetadata.institutionCode || this.customerMetadata.instCode || null,
            instCode: this.customerMetadata.institutionCode || this.customerMetadata.instCode || null,
    ...
    updateMetadataFields() {
        ...
        if (this.elements.instCode) {
            this.elements.instCode.textContent = this.customerMetadata.institutionCode || this.customerMetadata.instCode || '—';
        }
    ...
    checkCustomerDataCompleteness() {
        ...
        if (typeof CustomerMasterManager !== 'undefined') {
            const masterCustomer = CustomerMasterManager.findByAccountNumber(institutionCode, accountName);
            if (masterCustomer) {
                // Check if master customer has all critical fields
                const masterHasAllFields = (masterCustomer.custID || masterCustomer.customerId) &&
                    (masterCustomer.instID || masterCustomer.institutionId) &&
    ...
    showMessageHoverPopup(message, event) {
        ...
        // Subject/title
        if (message.text) {
            popupHtml += `<div class="message-preview-title">${this.escapeHtml(message.text)}</div>`;
        }
        
        // Description
        if (message.description) {
            popupHtml += `<div class="message-preview-content">${this.escapeHtml(message.description)}</div>`;
        }
        
        // Hover image
        if (message.hoverImage) {
            popupHtml += `<img class="message-preview-image" src="${this.escapeHtml(message.hoverImage)}" alt="Message image">`;
        }
    ...
    updateEnvironmentButtonVisibility() {
        ...
        envButton.style.display = shouldShow ? 'flex' : 'none';
        console.log('[PersistentBanner] Environment button visibility:', shouldShow);
    },
