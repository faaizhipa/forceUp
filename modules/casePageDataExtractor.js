...
const CasePageDataExtractor = {
  ...
  init() {
    ...
    if (
      typeof PageIdentifier !== "undefined" &&
      typeof PageIdentifier.monitorPageChanges === "function"
    ) {
      console.log(
        "[CasePageDataExtractor] Using PageIdentifier for page monitoring"
      );
      ...
      console.warn(
        "[CasePageDataExtractor] PageIdentifier not available, module will not function"
      );
    }
  },
  ...
  async handlePageChange(pageInfo) {
    ...
    const contextCaseNumber =
      context?.caseNumber || pageInfo.caseNumber || null;

    if (!caseId) {
      console.warn(
        "[CasePageDataExtractor] Case page detected but no case ID found"
      );
    ...
    if (this.currentCaseId && this.currentCaseId !== caseId) {
      console.log(
        `[CasePageDataExtractor] Case changed from ${this.currentCaseId} to ${caseId}, cleaning up...`
      );
    ...
    if (this.isExtracting && this.currentCaseId === caseId) {
      console.log(
        "[CasePageDataExtractor] Extraction already in progress for case:",
        caseId
      );
      return;
    }
    ...
    try {
      ...
      if (!this.isTokenActive(extractionToken)) {
        console.log(
          "[CasePageDataExtractor] Navigation changed during wait, aborting extraction"
        );
        return;
      }
      ...
      if (!this.isTokenActive(extractionToken)) {
        console.log(
          "[CasePageDataExtractor] Token no longer active after extraction, aborting"
        );
        return;
      }
      ...
      if (
        ...
        if (!validation.valid) {
          console.warn(
            `[CasePageDataExtractor] Extracted data validation failed: ${validation.reason}`
          );
          console.warn(
            `[CasePageDataExtractor] Data caseId: ${data.caseId}, caseNumber: ${data.caseNumber}`
          );
          if (validation.currentContext) {
            console.warn(
              `[CasePageDataExtractor] Current context caseId: ${validation.currentContext.caseId}, caseNumber: ${validation.currentContext.caseNumber}`
            );
          }
          ...
          if (isCaseNumberMismatch) {
            ...
            console.warn(
              `[CasePageDataExtractor] Extracted data is stale: data shows case ${data.caseNumber}, but current page shows case ${validation.currentContext.caseNumber}`
            );
            console.warn(
              `[CasePageDataExtractor] Clearing extraction state to allow fresh re-extraction`
            );
          ...
          if (
            ...
            console.log(
              "[CasePageDataExtractor] Validation failed but case IDs match. Retrying validation after delay..."
            );
            ...
            this.retryTimeoutId = setTimeout(async () => {
              try {
                ...
                if (currentCaseId !== retryCaseId) {
                  console.log(
                    "[CasePageDataExtractor] Case changed during retry, aborting"
                  );
                  this.retryTimeoutId = null;
                  this.isExtracting = false;
                  this.currentExtractionToken = null;
                ...
                const retryResult =
                  retryValidation instanceof Promise
                    ? await retryValidation
                    : retryValidation;

                if (retryResult.valid) {
                  console.log(
                    "[CasePageDataExtractor] Retry validation succeeded"
                  );
                  ...
                  console.warn(
                    `[CasePageDataExtractor] Retry validation also failed: ${retryResult.reason}`
                  );
                ...
                console.error(
                  "[CasePageDataExtractor] Error during retry validation:",
                  error
                );
                this.isExtracting = false;
                this.currentExtractionToken = null;
        ...
        if (validation.currentContext) {
          ...
          if (validation.currentContext.caseNumber) {
            ...
            if (data.caseNumber !== contextCaseNumber) {
              console.log(
                "[CasePageDataExtractor] Case number validated, re-extracting subject to ensure match"
              );
              const headerData =
                this.extractCaseNumberAndSubjectFromVisibleHeader();
              if (
                headerData.caseNumber === data.caseNumber &&
                headerData.subject
              ) {
                data.subject = headerData.subject;
                console.log(
                  "[CasePageDataExtractor] Re-extracted subject from visible header:",
                  data.subject
                );
              } else {
                // Clear subject if we can't get it from the matching header
                console.warn(
                  "[CasePageDataExtractor] Could not extract matching subject, clearing to prevent stale data"
                );
                data.subject = null;
              }
            }
  ...
  waitForPageLoad(token = null) {
    return new Promise((resolve) => {
      ...
      const checkElements = () => {
        ...
        const recordLayoutCandidates = document.querySelectorAll(
          "records-record-layout-item"
        );
        if (
          typeof CaseDomUtils !== "undefined" &&
          CaseDomUtils.isElementVisible
        ) {
          hasVisibleRecordLayout =
            CaseDomUtils.getFirstVisibleElement(recordLayoutCandidates) !==
            null;
        ...
        const flexipageCandidates =
          document.querySelectorAll("flexipage-field");
        if (
          typeof CaseDomUtils !== "undefined" &&
          CaseDomUtils.isElementVisible
        ) {
          hasVisibleFlexipageField =
            CaseDomUtils.getFirstVisibleElement(flexipageCandidates) !== null;
        ...
        if (
          ...
          if (hasVisibleRecordLayout || hasVisibleFlexipageField) {
            console.log(
              "[CasePageDataExtractor] Visible page elements ready (attempt " +
                attempts +
                ")"
            );
          } else {
            console.log(
              "[CasePageDataExtractor] Max attempts reached, proceeding anyway (attempt " +
                attempts +
                ")"
            );
          }
  ...
  getLastModifiedDate() {
    ...
    if (field) {
      ...
      const valueCandidates = field.querySelectorAll(
  ...
  deriveServerFromEnvironment(affectedEnvironment) {
    ...
    const match = affectedEnvironment.match(
      /(NA\d{2}|EU\d{2}|AP\d{2}|CN\d{2}|CA\d{2})/i
    );
  ...
  applyCustomerLookups(data) {
    const institutionCandidate =
      data.institutionCode || data.exLibrisAccountNumber || data.accountCode;
    const accountName = data.accountName;
    let customerInfo = null;

    if (typeof CustomerDataManager !== "undefined") {
      customerInfo = CustomerDataManager.findByInstitutionCode(
    ...
    if (!customerInfo && typeof CustomerMasterManager !== "undefined") {
      if (institutionCandidate) {
        customerInfo = CustomerMasterManager.findByInstitutionCode(
          institutionCandidate,
          accountName
        );
      } else if (
        accountName &&
        typeof CustomerMasterManager.findByAccountName === "function"
      ) {
    ...
    if (customerInfo) {
      ...
      if (!data.server && customerInfo.server) {
        ...
        data.server &&
        customerInfo.server &&
        data.server.toLowerCase() !== customerInfo.server.toLowerCase()
      ) {
        console.warn(
  ...
  normalizeApiData(apiData, caseId) {
    console.log(
      "[CasePageDataExtractor] Normalizing API data, field count:",
    ...
    const data = {
      ...
      platformService:
        apiData.PQ_Product_Group__c || apiData.Platform__c || null,
      ...
      accountCode:
        apiData.Ex_Libris_Account_Number__c || apiData.AccountCode__c || null,
      institutionCode:
        apiData.Institution_Code__c || apiData.InstCode__c || null,
      ...
      customerExLibrisAccountNumber:
        apiData.Ex_Libris_Account_Number__c || null,
    ...
    const derivedServer = this.deriveServerFromEnvironment(
      data.affectedEnvironment
    );
    if (derivedServer) {
  ...
  extractCaseNumberAndSubjectFromVisibleHeader() {
    ...
    if (
      !headerText &&
      typeof CaseDomUtils !== "undefined" &&
      typeof CaseDomUtils.getVisibleHighlightsContainer === "function"
    ) {
      ...
      if (container) {
        ...
        if (
          ...
          (typeof CaseDomUtils === "undefined" ||
            !CaseDomUtils.isElementVisible ||
            CaseDomUtils.isElementVisible(headerElement))
        ) {
    ...
    if (caseNumber && !subject && headerText) {
      ...
      console.warn(
        "[CasePageDataExtractor] Found case number but no subject in header:",
        headerText
      );
    }
  ...
  async extractAllCaseDataFromDOM(contextSnapshot = null) {
    ...
    const contextCaseNumber =
      snapshot?.caseNumber || headerData.caseNumber || null;
    ...
    const data = {
      ...
      // Customer data
      exLibrisAccountNumber: this.getRecordLayoutField(
      ...
      parentCaseOwner: this.getFlexipageField(
        "RecordParentCaseOwner_cField",
        true
      ),
      ...
      customerExLibrisAccountNumber: this.getFlexipageField(
        "RecordEx_Libris_Account_Number_cField",
        false
      ),
    ...
    if (
      ...
      if (customerInfo) {
        ...
        console.log(
          "[CasePageDataExtractor] Found customer by institution code:",
          customerInfo.name
        );
        console.log(
          "[CasePageDataExtractor] Using server from customer record:",
          customerInfo.server
        );
        console.log(
          "[CasePageDataExtractor] Applied customer data - custID:",
          customerInfo.custID,
          "instID:",
          customerInfo.instID,
          "server:",
        ...
        if (
          ...
          if (timezoneInfo && timezoneInfo.timezone) {
            ...
            data.timezoneDisplayName =
              ...
              timezoneInfo.timezone.replace(/_/g, " ");
            data.timezoneSource = timezoneInfo.source || "customerDataManager";
            console.log(
              "[CasePageDataExtractor] Timezone resolved via CustomerDataManager:",
              {
            ...
            console.log(
              "[CasePageDataExtractor] Timezone not found in CustomerDataManager for:",
              {
          ...
          console.warn(
            "[CasePageDataExtractor] CustomerDataManager not available for timezone resolution"
          );
        }
      } catch (error) {
        console.error(
          "[CasePageDataExtractor] Error resolving timezone:",
          error
        );
      }
    }
  ...
  getCaseNumber() {
    ...
    try {
      const flexValue = this.getFlexipageField(
        "RecordEx_Libris_Account_Number_cField",
        false
      );
      if (flexValue) {
      ...
      console.warn(
        "[CasePageDataExtractor] Error reading flexipage fallback for case number:",
        e
      );
    }
  ...
  getHeaderText() {
    ...
    const candidates = document.querySelectorAll(
      'slot[name="primaryField"] lightning-formatted-text, records-formula-output[slot="primaryField"] lightning-formatted-text'
    );
  ...
  getDescription() {
    ...
    const fieldCandidates = layoutItem.querySelectorAll(
      "lightning-formatted-text, .test-id__field-value"
    );
    let field = null;
    if (typeof CaseDomUtils !== "undefined" && CaseDomUtils.isElementVisible) {
    ...
    if (
      field.tagName &&
      field.tagName.toLowerCase() === "lightning-formatted-text"
    ) {
  ...
  getRecordLayoutField(label) {
    ...
    if (!layoutItem) {
      ...
      candidates.forEach((candidate) => {
        const shadowMatch = this.queryShadowDOM(
          `records-record-layout-item[field-label="${label}"]`,
          candidate
        );
        if (shadowMatch) {
    ...
    if (!layoutItem) {
      const partialCandidates = document.querySelectorAll(
        `records-record-layout-item[field-label*="${label}"]`
      );
      if (
        typeof CaseDomUtils !== "undefined" &&
      ...
      if (!layoutItem) {
        ...
        if (
          ...
          layoutItem = CaseDomUtils.getFirstVisibleElement(
            shadowPartialCandidates
          );
    ...
    if (!layoutItem) {
      console.warn(
        `[CasePageDataExtractor] Could not find visible layout item for label: "${label}"`
      );
      return null;
    }
    ...
    console.log(
      `[CasePageDataExtractor] Extracted "${label}" from visible element:`,
      value
    );
    return value;
  },
  ...
  extractRecordLayoutValue(layoutItem) {
    ...
    if (typeof CaseDomUtils !== "undefined" && CaseDomUtils.isElementVisible) {
      if (!CaseDomUtils.isElementVisible(layoutItem)) {
        console.warn(
          "[CasePageDataExtractor] Skipping extraction from non-visible layout item"
        );
        return null;
      }
    }
    ...
    if (!formattedText && layoutItem.shadowRoot) {
      formattedText = layoutItem.shadowRoot.querySelector(
    ...
    if (!formattedText) {
      formattedText = this.queryShadowDOM(
        "lightning-formatted-text",
        layoutItem
      );
    }
    ...
    let control = layoutItem.querySelector(
      "div.slds-form-element__control span"
    );
    if (!control && layoutItem.shadowRoot) {
      control = layoutItem.shadowRoot.querySelector(
        "div.slds-form-element__control span"
      );
    }
    if (!control) {
      control = this.queryShadowDOM(
        "div.slds-form-element__control span",
        layoutItem
      );
    }
  ...
  getFlexipageField(fieldId, isAnchored = false) {
    ...
    const visibleTabContainer = document.querySelector(
      'section.tabContent.active .forcegenerated-record-layout2[style*="display: block"]'
    );
    ...
    if (visibleTabContainer) {
      const tabCandidates = visibleTabContainer.querySelectorAll(
        `flexipage-field[data-field-id="${fieldId}"]`
      );
      if (
        typeof CaseDomUtils !== "undefined" &&
    ...
    if (!field) {
      console.warn(
        `[CasePageDataExtractor] Could not find visible flexipage-field with data-field-id: "${fieldId}"`
      );
      return null;
    }
    ...
    if (typeof CaseDomUtils !== "undefined" && CaseDomUtils.isElementVisible) {
      if (!CaseDomUtils.isElementVisible(field)) {
        console.warn(
          `[CasePageDataExtractor] Selected flexipage-field "${fieldId}" is not visible`
        );
        return null;
      }
    }
    ...
    if (isAnchored) {
      ...
      if (anchor) {
        ...
        if (spans.length > 0) {
          ...
          for (let i = spans.length - 1; i >= 0; i--) {
            ...
            if (text && text.length > 0) {
              ...
              if (childSpans.length === 0) {
                console.log(
                  `[CasePageDataExtractor] Extracted anchored field "${fieldId}":`,
                  text
                );
                return text;
              }
          ...
          if (lastSpan.textContent) {
            ...
            console.log(
              `[CasePageDataExtractor] Extracted anchored field "${fieldId}" (fallback):`,
              text
            );
            return text;
          }
        ...
        if (anchor.textContent) {
          ...
          console.log(
            `[CasePageDataExtractor] Extracted anchored field "${fieldId}" (anchor text):`,
            text
          );
          return text;
        }
      ...
      if (lookup && lookup.textContent) {
        ...
        console.log(
          `[CasePageDataExtractor] Extracted anchored field "${fieldId}" from force-lookup:`,
          text
        );
        return text;
      }
      ...
      console.warn(
        `[CasePageDataExtractor] Could not extract value from anchored field "${fieldId}"`
      );
    } else {
      ...
      if (!formattedText && field.shadowRoot) {
        formattedText = field.shadowRoot.querySelector(
          "lightning-formatted-text"
        );
      }
      if (!formattedText) {
      ...
      if (formattedText && formattedText.textContent) {
        ...
        console.log(
          `[CasePageDataExtractor] Extracted non-anchored field "${fieldId}":`,
          text
        );
        return text;
      }
      ...
      if (!control && field.shadowRoot) {
        control = field.shadowRoot.querySelector(
      ...
      if (control && control.textContent) {
        ...
        console.log(
          `[CasePageDataExtractor] Extracted non-anchored field "${fieldId}" from control:`,
          text
        );
        return text;
      }
      ...
      console.warn(
        `[CasePageDataExtractor] Could not extract value from non-anchored field "${fieldId}"`
      );
    }
  ...
  getFlexipageFieldWithFallback(
    ...
    for (const label of fallbackLabels) {
      ...
      if (value) {
        console.log(
          `[CasePageDataExtractor] Using fallback layout field "${label}" for ${fieldId}:`,
          value
        );
        return value;
      }
  ...
  async dispatchDataExtractedEvent(data) {
    ...
    if (
      ...
      if (!validation.valid) {
        console.warn(
          `[CasePageDataExtractor] Cannot dispatch event: ${validation.reason}`
        );
        return;
      }
    }
  ...
  getCaseIdFromUrl() {
    if (
      typeof PageContextValidator !== "undefined" &&
      typeof PageContextValidator.getCaseIdFromUrl === "function"
    ) {
      return PageContextValidator.getCaseIdFromUrl();
    }
    // Fallback implementation
    const match = window.location.pathname.match(
      /\/Case\/([a-zA-Z0-9]{15,18})\//
    );
  ...
  cleanup(reason = "manual") {
    ...
    if (window.ExLibrisExtension) {
      if (
        ...
        (reason === "case-switch" || reason === "context-switch")
      ) {
  ...
  async extractNow(force = false) {
    ...
    if (force) {
      console.log(
        "[CasePageDataExtractor] Force extraction triggered for case:",
        this.currentCaseId
      );
      this.lastExtractedData = null; // Clear cache
    } else {
      console.log(
        "[CasePageDataExtractor] Manual extraction triggered for case:",
        this.currentCaseId
      );
    }
