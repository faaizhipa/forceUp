  ...
  async extractAllCaseDataFromDOM(contextSnapshot = null) {
    ...
    if (typeof CustomerMasterManager !== 'undefined' && data.exLibrisAccountNumber) {
      ...
      if (customerInfo) {
        ...
        data.instCode = customerInfo.institutionCode || data.exLibrisAccountNumber; // Alias
        ...
        console.log('[CasePageDataExtractor] Applied customer data - custID:', data.custID, 'instID:', data.instID, 'server:', data.server, 'instCode:', data.instCode);
      } else {
        // No customer match - still set institutionCode from exLibrisAccountNumber
        data.institutionCode = data.exLibrisAccountNumber;
        data.instCode = data.exLibrisAccountNumber;
        console.log('[CasePageDataExtractor] No customer match found - using exLibrisAccountNumber as instCode:', data.instCode);
      }
