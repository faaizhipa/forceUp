...
(function() {
  ...
  const HighlighterController = {
    ...
    // Message section state (shared with persistentBanner storage)
    messageState: {
      messages: [],           // Active messages from storage
      currentIndex: 0,        // Current message index
      storageKey: 'exl_bannerMessages',
      indexStorageKey: 'exl_hl_messageIndex',  // Persisted message index
      rotationInterval: null, // Auto-rotation timer
      rotationDelay: 8000     // 8 seconds between rotations
    },
    ...
    radialMenuState: {
      ...
      menuElement: null,
      items: [],
      arcAngle: 210,      // Degrees of arc (slightly more than semi-circle)
      startAngle: -90,    // Start from top (pointing left when button on right)
      radius: 90,         // Distance from center (increased from 70)
      innerRadius: 58,    // Inner radius for color palette (increased from 45)
    ...
    async init() {
      ...
      try {
        ...
        // Initialize message system (shared with persistentBanner)
        await this.initMessageSystem();
    ...
    createBanner() {
      ...
      manageBtn.className = 'exl-hl-btn';
      manageBtn.innerHTML = '📋 Manage';
      manageBtn.title = 'Manage highlights and notes';
      manageBtn.addEventListener('click', () => {
        if (typeof HighlightsSidepanel !== 'undefined') {
          HighlightsSidepanel.togglePanel();
        }
      });
      ...
      actionsSection.appendChild(manageBtn);
      container.appendChild(actionsSection);

      // Section 6: Messages (shared with persistentBanner)
      ...
      const startDrag = (e) => {
        ...
        const onMouseMove = (e) => {
          ...
          if (deltaX > clickThreshold || deltaY > clickThreshold) {
            ...
            // Update radial menu position if expanded (follow the button)
            if (this.radialMenuState.isExpanded && this.radialMenuState.menuElement) {
              const centerX = newLeft + buttonWidth / 2;
              const centerY = newTop + buttonHeight / 2;
    ...
    async showFloatingButton() {
      ...
      // GUARD: Remove any orphaned buttons in DOM (prevents dual rendering)
      const existingButtons = document.querySelectorAll('.exl-hl-floating-btn');
      if (existingButtons.length > 0) {
        console.log(`[HighlighterController] Found ${existingButtons.length} existing button(s), cleaning up`);
        existingButtons.forEach(btn => {
          if (btn !== this.floatingButtonElement) {
            btn.remove();
          }
        });
      }
      
      // Don't create if already exists and is in DOM
      if (this.floatingButtonElement && document.body.contains(this.floatingButtonElement)) {
      ...
      // Clear stale reference if button is no longer in DOM
      if (this.floatingButtonElement && !document.body.contains(this.floatingButtonElement)) {
    ...
    showRadialMenu() {
      ...
      const arcAngle = this.radialMenuState.arcAngle; // Use state value (210°)
      ...
      const colorArcAngle = 190; // Slightly less than main arc (210) to nest inside
      const colorStartAngle = startAngle + 10; // Offset to center inside main arc
    ...
    // ================================================
    // MESSAGE SECTION (Shared with PersistentBanner)
    // ================================================

    /**
     * Initialize message system
     * Loads messages from storage and sets up listener
     */
    async initMessageSystem() {
      try {
        // Load messages from storage
        await this.loadMessages();
        
        // Load persisted message index
        const result = await chrome.storage.local.get([this.messageState.indexStorageKey]);
        if (result[this.messageState.indexStorageKey] !== undefined) {
          const savedIndex = result[this.messageState.indexStorageKey];
          // Ensure index is valid
          if (savedIndex >= 0 && savedIndex < this.messageState.messages.length) {
            this.messageState.currentIndex = savedIndex;
          }
        }
        
        // Set up storage listener for message updates
        chrome.storage.onChanged.addListener((changes, areaName) => {
          if (areaName === 'local' && changes[this.messageState.storageKey]) {
            console.log('[HighlighterController] Messages updated from storage');
        ...
        console.log(`[HighlighterController] Message system initialized with ${this.messageState.messages.length} messages`);
      } catch (error) {
        console.error('[HighlighterController] Error initializing message system:', error);
      }
    },

    /**
     * Load messages from storage (same storage as persistentBanner)
     */
    async loadMessages() {
      return new Promise((resolve) => {
        chrome.storage.local.get([this.messageState.storageKey], async (result) => {
          const messagesConfig = result[this.messageState.storageKey];
          ...
          // Get active messages (same logic as persistentBanner)
          this.messageState.messages = await this.getActiveMessages(messagesConfig);
          
          // Reset index if out of bounds
          if (this.messageState.currentIndex >= this.messageState.messages.length) {
            this.messageState.currentIndex = 0;
          }
    ...
    /**
     * Get active messages from config (matches persistentBanner logic)
     */
    async getActiveMessages(messagesConfig) {
      ...
      try {
        ...
        console.error('[HighlighterController] Error loading noted messages:', error);
      }

      // Add enabled default messages
      if (messagesConfig.defaultMessages?.enabled && messagesConfig.defaultMessages?.items) {
        messagesConfig.defaultMessages.items.forEach(msg => {
          if (notedMessages[msg.id]?.noted === true) return;
    ...
    /**
     * Create message section element for banner
     * @returns {HTMLElement} Message section element
     */
    createMessageSection() {
      const section = document.createElement('div');
      section.className = 'exl-banner-section exl-hl-message-section';
      
      // Message container
      const messageContainer = document.createElement('div');
      messageContainer.className = 'exl-hl-message-container';
      
      // Previous button
      const prevBtn = document.createElement('button');
      prevBtn.className = 'exl-hl-message-nav-btn exl-hl-message-prev';
      prevBtn.innerHTML = '◀';
      ...
      messageWrapper.className = 'exl-hl-message-wrapper';
      
      // Message text
      const messageText = document.createElement('span');
      messageText.className = 'exl-hl-message-text';
      messageText.textContent = 'No messages';
      
      // Message description (shown if space allows)
      const messageDesc = document.createElement('span');
      messageDesc.className = 'exl-hl-message-description';
      messageDesc.textContent = '';
      
      messageWrapper.appendChild(messageText);
      messageWrapper.appendChild(messageDesc);
      
      // Next button
      const nextBtn = document.createElement('button');
      nextBtn.className = 'exl-hl-message-nav-btn exl-hl-message-next';
      nextBtn.innerHTML = '▶';
      ...
      counter.className = 'exl-hl-message-counter';
      counter.textContent = '';
      
      messageContainer.appendChild(prevBtn);
      ...
      // Initial update
      this.updateMessageSectionContent(section);
      
      return section;
    },

    /**
     * Update message section content
     * @param {HTMLElement} section - Message section element (optional, will find if not provided)
     */
    updateMessageSectionContent(section) {
      // Find sections in both banners
      const sections = section ? [section] : [
        this.bannerElement?.querySelector('.exl-hl-message-section'),
        this.floatingBannerElement?.querySelector('.exl-hl-message-section')
      ].filter(Boolean);
      
      if (sections.length === 0) return;
      
      const messages = this.messageState.messages;
      const currentIndex = this.messageState.currentIndex;
      const hasMessages = messages.length > 0;
      
      sections.forEach(sec => {
        const textEl = sec.querySelector('.exl-hl-message-text');
        const descEl = sec.querySelector('.exl-hl-message-description');
        const counterEl = sec.querySelector('.exl-hl-message-counter');
        const prevBtn = sec.querySelector('.exl-hl-message-prev');
        const nextBtn = sec.querySelector('.exl-hl-message-next');
        
        if (hasMessages) {
          const currentMsg = messages[currentIndex];
          if (textEl) textEl.textContent = currentMsg.text;
          if (descEl) {
            descEl.textContent = currentMsg.description || '';
            descEl.style.display = currentMsg.description ? '' : 'none';
          }
          if (counterEl) {
            counterEl.textContent = messages.length > 1 ? `${currentIndex + 1}/${messages.length}` : '';
          }
          
          // Show/hide nav buttons based on message count
          const showNav = messages.length > 1;
          if (prevBtn) prevBtn.style.display = showNav ? '' : 'none';
          if (nextBtn) nextBtn.style.display = showNav ? '' : 'none';
        } else {
          if (textEl) textEl.textContent = 'No messages';
          if (descEl) {
            descEl.textContent = '';
     ...
     */
    showPreviousMessage() {
      if (this.messageState.messages.length <= 1) return;
      
      this.messageState.currentIndex = 
        (this.messageState.currentIndex - 1 + this.messageState.messages.length) % 
        this.messageState.messages.length;
      
      this.persistMessageIndex();
      this.updateMessageDisplay();
    },
    ...
    /**
     * Persist message index to storage (for cross-site persistence)
     */
    async persistMessageIndex() {
      try {
        await chrome.storage.local.set({
          [this.messageState.indexStorageKey]: this.messageState.currentIndex
        });
      } catch (error) {
        console.error('[HighlighterController] Error persisting message index:', error);
      }
    },

    /**
     * Update message display in all banners
     */
    updateMessageDisplay() {
      this.updateMessageSectionContent();
    },
    ...
    startMessageRotation() {
      ...
      this.messageState.rotationInterval = setInterval(() => {
        this.showNextMessage();
      }, this.messageState.rotationDelay);
    },
     ...
     */
    stopMessageRotation() {
      if (this.messageState.rotationInterval) {
        clearInterval(this.messageState.rotationInterval);
        this.messageState.rotationInterval = null;
      }
    },
