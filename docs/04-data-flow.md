...
- [Cross-World Data Flow (CRITICAL)](#cross-world-data-flow-critical)
...
│   • Intercepts Salesforce API responses                  │
│   • Captures case data BEFORE DOM renders                │
│   • Dispatches CustomEvent to ISOLATED world             │
└────────────────────┬─────────────────────────────────────┘
                     │ CustomEvent: 'EXLIBRIS_DATA_UPDATED'
                     ▼
┌──────────────────────────────────────────────────────────┐
│ ISOLATED WORLD (content_script_exlibris.js)              │
│   │                                                      │
│   ├─► InterceptorCacheManager (stores API data)          │
│   │                                                      │
...
Data Extraction (with fallback)
   ├─► PRIMARY: InterceptorCacheManager (API data)
   │   └─► Already captured by interceptor.js
   │
   └─► FALLBACK: CasePageDataExtractor (DOM scraping)
...
## Cross-World Data Flow (CRITICAL)

### Understanding Chrome Extension Worlds

This extension uses **two JavaScript execution contexts** (worlds) that CANNOT directly share variables:
...
│  ┌────────────────────────────────────────────────────┐   │
│  │              MAIN WORLD                            │   │
│  │  • interceptor.js runs here                        │   │
│  │  • window.exLibrisFetchedCaseData (global)         │   │
│  │  • Can intercept XHR/fetch requests                │   │
│  │  • Full access to page's JavaScript context        │   │
│  │  • ⚠️ NO access to chrome.* APIs                  │   │
...
│  ┌────────────────────────────────────────────────────┐   │
│  │            ISOLATED WORLD                          │   │
│  │  • content_script_exlibris.js runs here            │   │
...
### Interceptor Data Flow (Primary Source)

```
┌──────────────────────────────────────────────────────────┐
│ STEP 1: API INTERCEPTION (MAIN WORLD)                    │
└──────────────────────────────────────────────────────────┘
   ...
   interceptor.js (MAIN world, document_start)
          │
          ├─► Monkey-patches XMLHttpRequest.prototype.send
          │
          └─► Listens for '/aura' responses
                   │
                   ▼
   Response contains full case record:
   {
     CaseNumber: '00001234',
     Status: 'Open',
     Account: { Name: 'Customer Corp', ExLibrisAccountNum__c: '12345' },
     ...50+ fields from API
   }
          │
          ▼
   Store in MAIN world global:
   window.exLibrisFetchedCaseData.currentPage = apiData;
          │
          ▼
┌──────────────────────────────────────────────────────────┐
│ STEP 2: CROSS-WORLD BRIDGE (CustomEvent)                 │
└──────────────────────────────────────────────────────────┘
          │
          ▼
   Dispatch CustomEvent (only way to cross worlds):
   
   window.dispatchEvent(new CustomEvent('EXLIBRIS_DATA_UPDATED', {
     detail: {
       type: 'CASE_DATA',
       caseId: caseId,
       ...
       data: normalizedData
          ...
          ▼
┌──────────────────────────────────────────────────────────┐
│ STEP 3: ISOLATED WORLD RECEIVES DATA                     │
└──────────────────────────────────────────────────────────┘
          │
          ▼
   content_script_exlibris.js (ISOLATED world)
   
   window.addEventListener('EXLIBRIS_DATA_UPDATED', (event) => {
     const apiData = event.detail;
     
     // Store in InterceptorCacheManager (uses sessionStorage)
     InterceptorCacheManager.store(apiData.caseNumber, apiData);
     
     // Data now available for CasePageDataExtractor
   });
          │
          ▼
┌──────────────────────────────────────────────────────────┐
│ STEP 4: DATA ENRICHMENT & DISTRIBUTION                   │
└──────────────────────────────────────────────────────────┘
          │
          ▼
   CasePageDataExtractor checks InterceptorCacheManager FIRST
          │
          ├─► Cache HIT: Use API data (faster, more complete)
          │
          └─► Cache MISS: Fall back to DOM extraction
          │
          ▼
   Enrich with CustomerMasterManager (timezone, server, etc.)
          │
          ▼
   Dispatch 'casePageDataExtracted' event to all subscribers
```

### Data Source Priority

| Priority | Source | Timing | Completeness | Reliability |
|----------|--------|--------|--------------|-------------|
| 1 | Interceptor (API) | Pre-DOM render | Complete (50+ fields) | High |
| 2 | DOM Extraction | Post-DOM render | Partial (visible fields) | Medium |
| 3 | Customer Database | On-demand | Enrichment only | High |

### Why Two Sources?

1. **Interceptor (Primary)**: 
   - Captures ALL fields from API response
   - Available before DOM renders
   - Includes fields not displayed on page
   - Faster than DOM parsing

2. **DOM Extraction (Fallback)**:
   - Works when interceptor misses data
   - Required for fields modified client-side
   - Handles edge cases (cache miss, timing issues)

### Critical Rules for Cross-World Development

1. **Variables are NOT shared between worlds**
   ```javascript
   // ❌ WRONG: Cannot access MAIN world variables
   const data = window.exLibrisFetchedCaseData; // undefined in ISOLATED!
   
   // ✅ CORRECT: Use CustomEvent bridge
   window.addEventListener('EXLIBRIS_DATA_UPDATED', (e) => {
     const data = e.detail;
   });
   ```

2. **Interceptor runs BEFORE content scripts**
   - `interceptor.js`: `document_start` in MAIN world
   - Content scripts: `document_idle` in ISOLATED world
   - Data may be captured before listeners are ready
   - Use `window._pendingFetchedCaseData` pattern for early data

3. **CustomEvents are the ONLY bridge mechanism**
   - Direct function calls across worlds are IMPOSSIBLE
   - window.postMessage() works but CustomEvent is cleaner
   - DOM is shared but JavaScript context is not
...
┌─────────────────────────────────────────────────────┐
│                 EXTRACTION LAYER                    │
│  InterceptorCacheManager (API data, primary)        │
│  CasePageDataExtractor → CaseDomUtils (fallback)    │
