const ExportImport = {
  ...
  parseText(txtText) {
    ...
    for (let i = 1; i < urlSections.length; i++) {
      ...
      // Split each note block by separator line
      const noteSections = content.split('-------------------');
      for (let j = 0; j < noteSections.length; j++) {
        const noteBlock = noteSections[j].trim();
        if (!noteBlock) continue;

        const titleMatch = noteBlock.match(/Title: (.*)/);
        const dateMatch = noteBlock.match(/Date: (.*)/);
        const contentMatch = noteBlock.match(/Content:\n([\s\S]*)/);

        const title = titleMatch ? titleMatch[1].trim() : 'Untitled Note';
        const createdAt = dateMatch ? new Date(dateMatch[1]).getTime() || Date.now() : Date.now();
  ...
  async mergeData(importedData) {
    // Merge imported data with existing notes
    const existing = await Storage.getAllNotes();
    const merged = { ...existing };

    for (const [url, notes] of Object.entries(importedData)) {
      if (!merged[url]) merged[url] = [];

      notes.forEach(note => {
        // If note ID exists, update; else add
        const idx = merged[url].findIndex(n => n.id === note.id);
        if (idx > -1) {
          merged[url][idx] = { ...merged[url][idx], ...note };
        } else {
          merged[url].push(note);
        }
      });
    }

    await Storage.setAllNotes(merged);
  }
};
